	...
	public boolean transform() {
		...
		provider.register(new ComparisonProvider() {
			...
			public boolean instanceOf(JavaValue target, Type type, Context context) {
				if (target.type().equals("Ljava/lang/Class;") && type.getDescriptor().equals("Ljava/lang/Class;"))
					return true;
				return false;
			}
		...
		if (getDeobfuscator().invaildClasses.isEmpty()) {
			System.out.println(
					"[Special] [ParamorphismTransformer] Paramorphism not found or option not enabled. Exiting (If you see folder class in your jar.then consider using V1 or else v2 )");
			return false;
		}
		...
		System.out.println("[Special] [ParamorphismTransformer] Removing Bad annotations.");

		// Remove annotations. Update : you should judge for yourself ->
			...
			if (classNode.invisibleAnnotations != null) {
				...
				while (itr.hasNext()) {
					...
					if (next.desc != null && next.desc.length() > 50) {
						...
						for (char c : next.desc.toCharArray()) {
			...
			for (MethodNode method : classNode.methods)
				for (AbstractInsnNode ain : method.instructions.toArray())
					if ((Modifier.isStatic(method.access) || method.name.equals("<init>"))
							&& ain.getOpcode() == Opcodes.NEW && ((TypeInsnNode) ain).desc.equals("java/lang/Object")
							&& ain.getNext() != null && ain.getNext().getOpcode() == Opcodes.DUP
							&& ain.getNext().getNext() != null
							&& ain.getNext().getNext().getOpcode() == Opcodes.INVOKESPECIAL
						...
						if (next.getNext() == null || !((next.getNext().getOpcode() >= Opcodes.IRETURN
								...
								|| next.getNext().getOpcode() == Opcodes.ATHROW))
							continue;
					...
					} else if (!Modifier.isStatic(method.access) && !method.name.equals("<init>")
							&& ain.getOpcode() == Opcodes.ALOAD && ((VarInsnNode) ain).var == 0 && ain.getNext() != null
							&& ain.getNext().getOpcode() == Opcodes.DUP && ain.getNext().getNext() != null
							&& ain.getNext().getNext().getOpcode() == Opcodes.IFNONNULL) {
		...
		System.out.println("[Special] [ParamorphismTransformer] Removing goto switchup.");
			...
			for (MethodNode method : classNode.methods) {
				...
				for (AbstractInsnNode ain : method.instructions.toArray()) {
					...
					while ((passed ? ain.getOpcode() == Opcodes.GOTO : ain instanceof JumpInsnNode)
							&& ((JumpInsnNode) ain).label.getNext() != null
							&& ((JumpInsnNode) ain).label.getNext().getOpcode() == Opcodes.GOTO) {
				...
				if (modified) {
					...
					if (method.tryCatchBlocks != null)
						method.tryCatchBlocks.removeIf(tryCatchBlockNode -> Utils
								.getNext(tryCatchBlockNode.start) == Utils.getNext(tryCatchBlockNode.end));
		...
		System.out.println("[Special] [ParamorphismTransformer] Decrypting Hide Access.");
		...
		for (ClassNode classNode : classNodes()) {
			MethodNode clinit = classNode.methods.stream().filter(m -> m.name.equals("<clinit>")).findFirst()
					.orElse(null);
			AbstractInsnNode first = clinit == null ? null : clinit.instructions.getFirst();
			if (first != null && first.getOpcode() == Opcodes.NEW && first.getNext() != null
					&& first.getNext().getOpcode() == Opcodes.DUP && first.getNext().getNext() != null
					&& first.getNext().getNext().getOpcode() == Opcodes.LDC
				...
				try {
					...
					byte[] encrypted = getDeobfuscator().getInputPassthrough()
							.get(((LdcInsnNode) first.getNext().getNext().getNext().getNext()).cst);
					if (encrypted == null) {
					...
					gzipStream.close();
					getDeobfuscator().getInputPassthrough()
							.remove(((LdcInsnNode) first.getNext().getNext().getNext().getNext()).cst);
					...
					ClassNode node = new ClassNode(); // node.name -> "?"
					reader.accept(node, ClassReader.SKIP_FRAMES);
					toRemove.add(node.interfaces.get(0));// Interface
					// Decrypt all
					JavaValue instance = JavaValue.valueOf(new Object());

					HashMap<MethodNode, AbstractInsnNode> pip = new HashMap<MethodNode, AbstractInsnNode>();
					...
					for (MethodNode method : node.methods) {
						...
						for (AbstractInsnNode ain : method.instructions.toArray())
							if (ain instanceof FieldInsnNode || ain instanceof MethodInsnNode) {
								mode = ain;
								break;
							} else if (ain instanceof LdcInsnNode && ain.getNext() != null
		...
		// Here we reverse all the method call back to normal
